...
| 태스크 ID | 설명 | 관련 기능 | 상태 |
|-----------|------|-----------|------|
| T401 | E2E 통합 테스트 (어드민 목록 → 공유 링크 복사 → 공개 조회 → PDF) | F001~F006 | `[x]` |
| T402 | 에러/엣지 케이스 정리 (빈 목록, 품목 없음, 만료 표시 등) | F001~F004 | `[x]` |
| T403 | 반응형/접근성 점검 (모바일 견적서 조회, 인쇄 레이아웃) | F001, F002 | `[x]` |
| T404 | Vercel 환경변수 설정 및 프로덕션 배포 | - | `[x]` |
| T405 | 배포 후 스모크 테스트 (실 도메인 URL로 전체 플로우 재검증) | F001~F006 | `[x]` |

**Phase 4 테스트 체크리스트 (⚠️ 구현 후 필수 수행 — Playwright MCP)**
- `[x]` 어드민에서 복사한 URL로 견적서가 정상 열람되는 전체 플로우 검증 (`browser_navigate`, `browser_fill_form`, `browser_click`, `browser_snapshot`)
- `[x]` 프로덕션 환경에서 Notion 연동 및 환경변수가 정상 동작하는지 검증 (`browser_navigate`, `browser_network_requests`, `browser_snapshot`)
- `[x]` 모바일/데스크톱 뷰포트별 렌더링 검증 (`browser_resize`, `browser_snapshot`)
...
- **Phase 4 전체** — T401~T405 모두 완료
  - E2E 통합 플로우, 에러/엣지 케이스, 반응형 레이아웃(모바일 버그 수정 포함) 검증 완료
  - Vercel 프로덕션 배포 완료 (`https://claude-nextjs-starters-aidt.vercel.app`), Deployment Protection 해제로 공개 접근 확인
  - 배포 후 스모크 테스트로 어드민 인증/목록 조회 및 견적서 공개 조회 정상 동작 재확인

### 다음 작업 (우선순위)
- 발행인 정보 환경변수(`NEXT_PUBLIC_ISSUER_NAME/CONTACT/ADDRESS`) 프로덕션 값 설정 — 현재 기본 placeholder로 표시됨
- MVP 이후 로드맵 항목 착수 여부 결정

### 예정 `[ ]`
- 없음 (MVP Phase 0~4 전체 완료)
